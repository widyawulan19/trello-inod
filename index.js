...
module.exports = { generateMarketingDesignNumbers };
...
// TESTING ENDPOIN EDIT DATA MARKETING DESIGN 
// ✅ UPDATE Data Marketing Design by ID (FINAL & SAFE)
    ...
    try {
      ...
      `,
            [
                ...
                file_and_chat,
                detail_project,
                emptyToNull(input_by),
                emptyToNull(acc_by),
                emptyToNull(account),
                emptyToNull(offer_type),
                emptyToNull(order_type_id),
                emptyToNull(project_type_id),
                emptyToNull(style_id),
                emptyToNull(status_project_id),
                resolution,
                reference,
                project_number ?? null,
                id,
        ...
        if (result.rowCount === 0) {
            return res.status(404).json({
                error: "❌ Marketing design not found",
            });
        }

        // ✅ AMBIL DATA JOIN (KONSISTEN DENGAN LIST)
        const joined = await client.query(
            `
      SELECT 
        md.marketing_design_id,
