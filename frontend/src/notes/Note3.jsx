        ...
        <ul className="space-y-3">
          {cardActivities.map(activity => {
            ...
            if (activity.action_type === 'move' && detail.cardTitle) {
              ...
              messageElement = (
                <>
                  <span className="font-semibold">{activity.username}</span> duplicated{' '}
                  <span className="font-bold">"{detail.cardTitle}"</span>
                </>
              );
            } else {
              // Default message
              const messageText = MESSAGE_ACTIVITY[activity.action_type] || 'performed an action';
              messageElement = (
                <>
